...
class CacheDataFrameLoader(DataFrameLoader):
    ...
    def __init__(self,cache_location:Optional[dict] = None) -> None:
        super().__init__()
        self.cache_location=cache_location or {"uri":os.getenv("DATAFRAME_CACHE_PATH","./dataframe_cache")}
    
    def load_data_frame(self,file_path:str,data_frame_location:Union[dict,LocationConfig]) -> pd.DataFrame:
        with open_location(self.cache_location) as (cache_file_system,cache_root_location):
            ...
            if cache_file_system.exists(cache_file_path):
                cached_data_frame = read_parquet(file_path=cache_file_path,file_system=cache_file_system)
            ...
            with open_location(data_frame_location) as (file_system,root_location): 
                ...
                data_frame = read_parquet(file_path=file_path,file_system=file_system)
                write_parquet(data_frame=data_frame,file_path=cache_file_path,file_system=cache_file_system)
